...
func MotifEnumeration(dnas []string, k int, d int) []string {
	// get all neighbors from all k-mers in the first dna string
	dna1 := dnas[0]
	motifsMap := make(map[string]bool)
	for i := 0; i < len(dna1)-k+1; i++ {
		pattern := dna1[i : i+k]
		for _, nb := range Neighbors(pattern, d) {
			motifsMap[nb] = true
		}
	}
	// repeat Set-intersection
	for _, dna := range dnas {
		// get all candidates from each dna string
		candidatesMap := make(map[string]bool)
		for i := 0; i < len(dna)-k+1; i++ {
			pattern := dna[i : i+k]
			for _, nb := range Neighbors(pattern, d) {
				candidatesMap[nb] = true
			}
		}
		// set intersection, modifying iterator during iteration
		// Modifying map while iterating over it in Go
		// https://stackoverflow.com/questions/68639365/modifying-map-while-iterating-over-it-in-go
		for key := range motifsMap {
			if _, ok := candidatesMap[key]; !ok {
				delete(motifsMap, key)
			}
		}
	}
	// get keys from result map
	result := []string{}
	for k := range motifsMap {
...
func main() {
	...
	// check time
	start := time.Now()
	for _, motif := range MotifEnumeration(dnas, k, d) {
		fmt.Print(motif, " ")
	}
	fmt.Println()
